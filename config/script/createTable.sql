...
COMMENT ON COLUMN users.firebase_uid IS 'Firebase 고유 UID';
...
COMMENT ON COLUMN feed.firebase_uid IS '작성자의 Firebase UID';
...
COMMENT ON COLUMN comment.firebase_uid IS '댓글 작성자의 Firebase UID';
...
COMMENT ON COLUMN like_status.firebase_uid IS '좋아요를 누른 사용자 Firebase UID';
...
CREATE TABLE notification (
    ...
    actor_id VARCHAR(128) REFERENCES users(firebase_uid) ON DELETE CASCADE,     
    target_id UUID,                                                 
    target_type INT,                                                
...
COMMENT ON COLUMN notification.receiver_id IS '알림 수신자의 Firebase UID';
COMMENT ON COLUMN notification.actor_id IS '알림 발생자의 Firebase UID';
